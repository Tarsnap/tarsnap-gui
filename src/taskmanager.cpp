                               ...
                               const QString &stdOut, const QString &stdErr)
{
    ...
    if(exitCode == SUCCESS)
    {
        emit message(tr("Cache repair succeeded."));
    }
    else
    {
        emit message(tr("Cache repair failed."));
        parseError(stdErr);
    }
    getArchives();
}
                               ...
                               const QString &stdOut, const QString &stdErr)
{
    ...
    if(exitCode == SUCCESS)
    {
        emit message(tr("All archives nuked successfully."));
        fsck(false);
    }
    else
    {
        emit message(tr("Archives nuke failed."));
        parseError(stdErr);
        return;
    }
}
